...
def constructBayesNet(gameState: hunters.GameState):
    ...
    """ 1)Add variables and edges based on the diagram.
        2)Pacman and the two ghosts can be anywhere in the grid (we ignore walls for this). Add all possible position tuples for these.
        3)Observations here are non-negative, equal to Manhattan distances of Pacman to ghosts ± noise. """
    "*** YOUR CODE HERE ***"
    # 1)Add variables and edges based on the diagram. edges(x,y)=x->y from x to y
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]
    # 2)Pacman and the two ghosts can be anywhere in the grid (we ignore walls for this). Add all possible position tuples for these.
    # since they are anywhere I need to make a combination for all widths and height in all coordinates
    MaxXcordinates = X_RANGE
    MaxYcordinates = Y_RANGE
    Allcoordinates = []
    for x in range(MaxXcordinates):
        for y in range(MaxYcordinates):
            Allcoordinates.append((x, y))
    variableDomainsDict[PAC] = Allcoordinates
    variableDomainsDict[GHOST0] = Allcoordinates
    variableDomainsDict[GHOST1] = Allcoordinates
    # 3)Observations here are non-negative, equal to Manhattan distances of Pacman to ghosts ± noise.
    """ 
    #   #   # 7 n   
    #   #   n 6 g 7 n   
    ...
    disty=4
    disty=4
    DistTotal=disty+disty+n-1
    """
    Maxrange = MaxXcordinates + MaxYcordinates + MAX_NOISE - 1
    ObservationRanges = []
    for x in range(Maxrange):
        ObservationRanges.append(x)

    variableDomainsDict[OBS0] = ObservationRanges
    variableDomainsDict[OBS1] = ObservationRanges
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        cpt_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for var in eliminationOrder:
            cpt_list, joined_factor = joinFactorsByVariable(cpt_list, var)
            if len(joined_factor.unconditionedVariables()) > 1:
                eliminated_factor = eliminate(joined_factor, var)
                cpt_list.append(eliminated_factor)

        joined_factor = joinFactors(cpt_list)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in allGhostPositions:
            observationProbability = self.getObservationProb(observation, pacPos, pos, jailPos)
            priorBelief = self.beliefs[pos]
            updatedBelief = observationProbability * priorBelief
            self.beliefs[pos] = updatedBelief
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPosition in self.allPositions:
            ...
            for newPosition, likelihood in newPosDist.items():
                newBeliefs[newPosition] += self.beliefs[oldPosition] * likelihood
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if observation is None:  # A ghost is captured
            newBelief = [self.getJailPosition()] * self.numParticles
        # print(self.particles)
        else:
            for particle in self.particles:
                weight = self.getObservationProb(observation, pacPosition, particle, self.getJailPosition())
                weights += weight
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newBelief = DiscreteDistribution()  # new distribution for the ghost position
        newParticle = []  # list of particles corresponding to each existing particle
        for position in self.allPositions:
            newBelief[position] = self.getPositionDistribution(gameState, position)
        for particle in self.particles:
            newParticle.append(newBelief[particle].sample())
